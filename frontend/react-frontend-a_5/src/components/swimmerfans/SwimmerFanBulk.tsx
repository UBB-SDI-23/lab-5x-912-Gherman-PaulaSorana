...
import { Paginator } from "../pagination/Pagination";
  ...
  const SwimmerFanBulk = () => {
    ...
    useEffect(() => {
      ...
      fetch(`${BACKEND_API_URL}/swimmerFan/?page=${page}&page_size=${new_page_size}`)
        .then((res) => res.json())
        .then((data) => {
    ...
    const handleRowSelection = (swimmerId: number) => {
      ...
      const handleBulkDelete = () => {
        axios({
            method: 'delete',
            url: `${BACKEND_API_URL}/swimmerFanBulk/${selectedRows.join(",")}/`,
            headers: {
            ...
            .then(() => {
              ...
              fetch(`${BACKEND_API_URL}/swimmerFan/?page=${page}&page_size=${rowsPerPage}`)
                .then(res => res.json())
                .then(data => {
                  const { count, next, prev, results } = data;
                  setIsLastPage(!next);
                    ...
                    <TableCell align="center">
                      <Button
                        ...
                        <TableRow key={swimmer.id}>
                            ...
                            <TableCell sx={{ color: "#2471A3" }} align="center">
                            {/* <Link to={`/swimmers/${swimmer.id}`}>{swimmer.fan_page_name}</Link> */}
                            </TableCell>
                            <TableCell align="center">
                            <Checkbox
