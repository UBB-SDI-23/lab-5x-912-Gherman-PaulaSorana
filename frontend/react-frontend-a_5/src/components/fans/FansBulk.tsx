...
import { Paginator } from "../pagination/Pagination";
  ...
  const FansBulk = () => {
    ...
    useEffect(() => {
      ...
      fetch(`${BACKEND_API_URL}/fan/?page=${page}&page_size=${new_page_size}`)
        .then((res) => res.json())
        .then((data) => {
    ...
    const handleRowSelection = (swimmerId: number) => {
      ...
      const handleBulkDelete = () => {
        axios({
            method: 'delete',
            url: `${BACKEND_API_URL}/fanBulk/${selectedRows.join(",")}/`,
            headers: {
            ...
            .then(() => {
              ...
              fetch(`${BACKEND_API_URL}/fan/?page=${page}&page_size=${rowsPerPage}`)
                .then(res => res.json())
                .then(data => {
                  const { count, next, prev, results } = data;
                  setIsLastPage(!next);
    ...
    return (
      <Container>
        ...
        {!loading && swimmers.length === 0 && <div>No fan found</div>}
                  ...
                  <TableRow>
                    ...
                    <TableCell align="center">
                      <Button
                        ...
                        <TableBody>
                        {swimmers.map((swimmer:Fan, index) => (
                        <TableRow key={swimmer.id}>
                            <TableCell sx={{ color: "#2471A3" }} align="center">
                            {(page - 1) * rowsPerPage + index + 1}
                            </TableCell>
                            <TableCell sx={{ color: "#2471A3" }} align="center">
                            <Link to={`/fans/${swimmer.id}`}>{swimmer.fan_first_name}</Link>
                            </TableCell>
                            <TableCell align="center">
                            <Checkbox
