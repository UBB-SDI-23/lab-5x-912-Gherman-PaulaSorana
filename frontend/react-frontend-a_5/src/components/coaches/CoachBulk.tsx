...
import { Paginator } from "../pagination/Pagination";
  ...
  const CoachesBulk = () => {
    ...
    useEffect(() => {
      ...
      fetch(`${BACKEND_API_URL}/coach/?page=${page}&page_size=${new_page_size}`)
        .then((res) => res.json())
        .then((data) => {
    ...
    const handleRowSelection = (swimmerId: number) => {
      ...
      const handleBulkDelete = () => {
        axios({
            method: 'delete',
            url: `${BACKEND_API_URL}/coachBulk/${selectedRows.join(",")}/`,
            headers: {
            ...
            .then(() => {
              ...
              fetch(`${BACKEND_API_URL}/coach/?page=${page}&page_size=${rowsPerPage}`)
                .then(res => res.json())
                .then(data => {
                  const { count, next, prev, results } = data;
                  setIsLastPage(!next);
    ...
    return (
      <Container>
        ...
        {!loading && swimmers.length === 0 && <div>No coach found</div>}
                  ...
                  <TableRow>
                    ...
                    <TableCell align="center">
                      <Button
                        ...
                        <TableBody>
                        {swimmers.map((swimmer:Coach, index) => (
                        <TableRow key={swimmer.id}>
                            <TableCell sx={{ color: "#2471A3" }} align="center">
                            {(page - 1) * rowsPerPage + index + 1}
                            </TableCell>
                            <TableCell sx={{ color: "#2471A3" }} align="center">
                            <Link to={`/fans/${swimmer.id}`}>{swimmer.coach_first_name}</Link>
                            </TableCell>
                            <TableCell align="center">
                            <Checkbox
